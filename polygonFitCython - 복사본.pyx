...
def getLineProfile(np.ndarray pts not None, np.ndarray imgArray not None, bool debug = False):
    cdef np.ndarray resultX
    cdef np.ndarray resultY
    cdef double slope = 0
    ...
    for i in range(len(newPts) - 1):        
        ...
        for i in range(startP, endP):
            y = slope * i + offset
            resultX = np.append(resultX, i, axis = 0)
            resultY = np.append(resultY, y, axis = 0)
...
def getStdOfArea(np.ndarray imgArray not None, np.ndarray OneMetric not None, bool debug = False):    
...
def cartesianToPolarcoord(np.ndarray x not None, np.ndarray y not None, np.ndarray centroid not None):
    x = x - centroid[0]
    y = y - centroid[1]
    rho = np.sqrt(x**2 + y**2)
    phi = np.arctan2(y, x)
...
def PolarToCartesian(np.ndarray r, np.ndarray p, np.ndarray centroid):
...
def calRegresssionByAxis(np.ndarray imgArray, np.ndarray pts, int ax, int direction, int ind, np.ndarray offsetLists, np.ndarray lrs, np.ndarray lrsStd, double alphaMean, double alphaStd ):
    ...
    oldStd, oldMean, oldSum = getStdOfArea(imgArray, getOneMetricForCalStd(imgArray, lineX, lineY), False)
    ...
    lineX, lineY = getLineProfile(pts, imgArray)
    newStd, newMean, newSum = getStdOfArea(imgArray, getOneMetricForCalStd(imgArray, lineX, lineY), False)
...
def pologonFit(np.ndarray originalPoints, double actureArea, str imagePath = False, int iterationLimit = 100, double alphaMean = 5, double alphaStd = 2, int direction = -1):
    '''
    Parameters
